...
export declare class SharedTransition {
  ...
  /**
   * Lets you create a shared transition animation bound to the progress
   * between navigation screens. Other shared transition modifiers can be
   * chained alongside this modifier.
   *
   * @param progressAnimationCallback - A callback called with the current
   *   progress value on every animation frame. It should return an object
   *   with styles for the shared transition.
   * @experimental
   * @see https://docs.swmansion.com/react-native-reanimated/docs/shared-element-transitions/overview
   */
  ...
  /**
   * Lets you adjust the behavior when the device's reduced motion
   * accessibility setting is turned on. Other shared transition modifiers can
   * be chained alongside this modifier.
   *
   * @param reduceMotion - Determines how the animation responds to the
   *   device's reduced motion accessibility setting. Default to
   *   `ReduceMotion.System` - {@link ReduceMotion}.
   * @experimental
   * @see https://docs.swmansion.com/react-native-reanimated/docs/shared-element-transitions/overview
   */
