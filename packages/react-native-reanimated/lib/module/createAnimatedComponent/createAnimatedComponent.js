...
export function createAnimatedComponent(Component, options) {
  ...
  class AnimatedComponent extends React.Component {
    ...
    constructor(props) {
      ...
      if (
        !entering ||
        getReducedMotionFromConfig(entering) ||
        skipEntering ||
        !isFabric()
      ) {
        return;
    ...
    componentDidMount() {
      ...
      if (IS_WEB) {
        ...
        if (
          !this.props.entering ||
          getReducedMotionFromConfig(this.props.entering)
        ) {
    ...
    componentWillUnmount() {
      ...
      if (
        ...
        this._componentDOMRef &&
        exiting &&
        !getReducedMotionFromConfig(exiting)
      ) {
    ...
    componentDidUpdate(
      ...
      if (
        ...
        snapshot !== null &&
        this.props.layout &&
        !getReducedMotionFromConfig(this.props.layout)
      ) {
        tryActivateLayoutTransition(
          this.props,
          this._componentDOMRef,
