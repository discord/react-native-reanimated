...
import { makeShareable, runOnUI, startMapper, stopMapper } from "../core.js";
...
For more, see the docs: \`https://docs.swmansion.com/react-native-reanimated/docs/guides/web-support#web-without-the-babel-plugin\`.`);
  ...
  useEffect(() => {
    ...
    if (isJest()) {
      ...
      if (Platform.OS === 'android' && !globalThis._IS_FABRIC && remoteState.isFirstRun) {
  ...
  if (!animatedStyleHandle.current) {
    ...
    if (__DEV__) {
      ...
      animatedStyleHandle.current = handle;
    } else {
