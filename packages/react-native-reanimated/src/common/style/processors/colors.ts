...
const isRecord = <T extends UnknownRecord = UnknownRecord>(
  value: unknown
): value is T =>
...
export function processColorNumber(value: unknown): number | null {
  ...
  return normalizedColor as unknown as number | null;
}
...
function unprocessDynamicColorObjectIOS(
  ...
  for (const property of DynamicColorIOSProperties) {
    if (value.dynamic[property] !== undefined) {
      result[property] = unprocessColorNumber(
        value.dynamic[property] as unknown as number
      );
    }
  }
